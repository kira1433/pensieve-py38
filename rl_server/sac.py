...
def CreateCriticNetwork(inputs, a_dim, name):
    with tf.compat.v1.variable_scope('Q_network' + name):
...
class Network():
    def __init__(self, sess, state_dim, action_dim, learning_rate, name):
        ...
        self.sess = sess
        self.inputs = tf.compat.v1.placeholder(tf.float32, [None, self.s_dim[0], self.s_dim[1]])
        self.q_target = tf.compat.v1.placeholder(tf.float32, [None, 1])
        ...
        self.actor_network_params = \
                tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope= self.name + '/actor')
        self.q1_network_params = \
                tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope= self.name + '/Q_network1')

        self.q2_network_params = \
                tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope= self.name + '/Q_network2')

        self.params = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES, scope=self.name)
        ...
        self.q1a_val = tf.compat.v1.reduce_sum(tf.multiply(self.q_net1, self.acts), reduction_indices=1, keepdims=True)
        self.q2a_val = tf.compat.v1.reduce_sum(tf.multiply(self.q_net2, self.acts), reduction_indices=1, keepdims=True)
        ...
        self.log_pi_prob = tf.compat.v1.log(self.actor_net)
        self.entropy = -tf.multiply(self.actor_net, tf.compat.v1.log(self.actor_net))
        self.min_q = tf.minimum(self.q_net1, self.q_net2)
        self.pi_loss = -tf.compat.v1.reduce_sum(self.entropy_weight*self.entropy + tf.multiply(tf.stop_gradient(self.min_q), self.actor_net))

        self.pi_optimize = tf.compat.v1.train.AdamOptimizer(self.lr_rate).minimize(self.pi_loss)
        self.q_optimize = tf.compat.v1.train.AdamOptimizer(self.lr_rate).minimize(self.q_loss)
...
class ReplayBuffer():
    ...
    def __init__(self, obs_dim, act_dim, size):
        self.obs1_buf = np.zeros([size, obs_dim[0], obs_dim[1]], dtype=np.float32)
        self.obs2_buf = np.zeros([size, obs_dim[0], obs_dim[1]], dtype=np.float32)
