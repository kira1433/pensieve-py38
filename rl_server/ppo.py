...
class Network():
    ...
    def r(self, pi_new, pi_old, acts):
        return tf.compat.v1.reduce_sum(tf.multiply(pi_new, acts), reduction_indices=1, keepdims=True) / \
                tf.compat.v1.reduce_sum(tf.multiply(pi_old, acts), reduction_indices=1, keepdims=True)
    ...
    def __init__(self, sess, state_dim, action_dim, learning_rate):
        ...
        self.old_pi = tf.compat.v1.placeholder(tf.float32, [None, self.a_dim])
        ...
        self.log_prob = tf.compat.v1.log(tf.compat.v1.reduce_sum(tf.multiply(self.real_out, self.acts), reduction_indices=1, keepdims=True))
        self.entropy = tf.multiply(self.real_out, tf.compat.v1.log(self.real_out))
