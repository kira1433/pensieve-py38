...
# NN_MODEL = None
NN_MODEL = '../rl_server/results/sac_ep_93800.ckpt'
...
def make_request_handler(input_dict):
    ...
    class Request_Handler(BaseHTTPRequestHandler):
        def __init__(self, *args, **kwargs):
            ...
            self.target_net = input_dict['target_net']
            self.eval_net = input_dict['eval_net']
        ...
        def do_POST(self):
            ...
            if ( 'pastThroughput' in post_data ):
                ...
                # --linear reward--
                linear_reward = VIDEO_BIT_RATE[post_data['lastquality']] / M_IN_K \
                ...
                log_reward = log_bit_rate \
                         - 4.3 * rebuffer_time / M_IN_K \
                         - SMOOTH_PENALTY * np.abs(log_bit_rate - log_last_bit_rate)

                # --hd reward--
                hd_reward = BITRATE_REWARD[post_data['lastquality']] \
                ...
                action_prob = self.target_net.get_action_prob(np.reshape(state, (1, S_INFO, S_LEN)))
                # print(action_prob)
...
# def run(server_class=HTTPServer, port=int(os.environ.get('PORT', 8000)), log_file_path=LOG_FILE):
def run(server_class=HTTPServer, port=8001, log_file_path=LOG_FILE):
    ...
    with tf.compat.v1.Session() as sess, open(log_file_path, 'wb') as log_file:
        target_net = sac.Network(sess, state_dim=S_DIM, action_dim=A_DIM, learning_rate=ACTOR_LR_RATE, name='target')
        eval_net = sac.Network(sess, state_dim=S_DIM, action_dim=A_DIM, learning_rate=ACTOR_LR_RATE, name='eval')
        ...
        input_dict = {'sess': sess, 'log_file': log_file,
                      'target_net': target_net, 'eval_net': eval_net,
